...
A Függelék fejezet első része a felhasznát matematikai eszközök összefoglalását tartalmazza az \cite{szego, szoke} könyveket alapul véve. A matematikai eszközök bemutatásán kívül szintén ebben a fejezetben található
...
Jelöljük az $\Bbb R$ számegyenesen szakaszonként folytonos, négyzetesen integrálható függvények terét $\mathcal F$-fel és vezessük be az $\mathcal F$ téren az
$$
...
\subsubsection{\it  Becslések az $F_n$ függvényre}
 ...
 \end{split}
 \end{equation}
 Ezeket felhasználva becsléseket adunk az
 $$
 ...
 $$
 Fourier-együtthatókra. A továbbiakban kompakt tartójú, korlátos függvényekből indulunk ki. Nem jelenti az általánosság megszorítását ha  feltesszük, hogy
 $$
 ...
 $$
 függvény folytonosan  differenciálható. Megmutatjuk, hogy az $F_n:T\to [0,\infty)$
...
\end{split}
\end{equation}
Hasonlóan egyenlőtlenséget kapunk az $a<-2\lambda$ esetben. Innen következik, hogy az
$$
T_s:=\{(p,q): -2s\le p\le 2s, 1/s\leq p \le s\}
$$
téglalapon kívüul  érvényes
$$
\sup_{(a,\lambda) \notin T_s}|A_k(a,\lambda)|\le \frac {8M_k}{\sqrt s}\ \  (s>1)
$$
becslés. Ennek alapján nyilvánvaló, hogy a $F_n$  függvénynek létezik a maximuma a $T$ téglelapon.
